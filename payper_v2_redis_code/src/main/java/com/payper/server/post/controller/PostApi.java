...
public interface PostApi {
    ...
    ResponseEntity<ApiResponse<Void>> deletePost(
            CustomUserDetails user, @Parameter(description = "게시글 ID", example = "1") Long postId);

    @Operation(
            summary = "게시글 상세 조회",
            description = "삭제되지 않은 게시글만 조회 가능",
            security = {})
    ...
    @Operation(
            summary = "게시글 목록 조회",
            description = "가맹점/타입 필터링, 정렬, 오프셋 기반 페이지네이션을 지원합니다.",
            security = {})
    ...
    @Operation(
            summary = "게시글 댓글 조회",
            description = "부모 댓글 기준 커서 페이지네이션. 삭제된 부모 댓글도 자식이 있으면 '[삭제된 댓글입니다]'로 표시됩니다.",
            security = {})
    ...
    @Operation(
            summary="게시글 좋아요 버튼 누르기",
            description = "좋아요, 좋아요 취소",
            security = {}
    )
    ResponseEntity<ApiResponse<Void>> likePost(
            @Parameter(description = "게시글 ID") Long postId,
            @Parameter(description = "테스트용 파라미터 유저 ID")Long userId
    );
}
